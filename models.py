...
class _PlainBlock(nn.Module):
    """Same params as _ResBlock (2x Linear(H,H)), but no residual connection."""
    def __init__(self, hidden_dim):
        super().__init__()
        self.lin1 = nn.Linear(hidden_dim, hidden_dim)
        self.lin2 = nn.Linear(hidden_dim, hidden_dim)
    ...
    def forward(self, h):
        return self.act(self.lin2(self.act(self.lin1(h))))  # no +h

class FluxNet_nores(nn.Module):
    """
    2D FluxNet model that represents a divergence-free and curl-free vector field
    """
    def __init__(self, coordinate_dims = 2, hidden_dim = 256, n_hidden_layers = 6):
        ...
        # Then n_hidden_layers residual blocks
        # e.g. 6 residual blocks
        self.trunk = nn.ModuleList([_PlainBlock(hidden_dim) for _ in range(n_hidden_layers)])
