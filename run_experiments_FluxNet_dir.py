...
for n_run in range(1, N_RUNS + 1): 
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for X_batch, Y_batch in train_loader:
            ...
            dg_output = fluxnet_model(batch_dg_in)
    ...
    if n_run == 1:
        # Save trained model
        torch.save(fluxnet_model.state_dict(), "trained_models/fluxnet-dir_trained_20epochs.pth")
        # Save training loss convergence
        pd.DataFrame({"train_loss": train_losses}).to_csv(
        "trained_models/fluxnet-dir_trained_20epochs_loss_convergence.csv", index = False)
...
#### END RUN LOOP ####
# Save run metrics
pd.DataFrame(run_metrics).to_csv("results/fluxnet-dir_runs_metrics.csv", index = False)